"""
Subprocess worker for measuring upstream chatterbox-tts generation timing.

ARCHITECTURAL CONSTRAINT:
    This file runs under the interpreter from CHATTERBOX_REGRESSION_BASELINE_PYTHON
    (a separate venv with the upstream chatterbox-tts). The fork's benchmarks package
    is not available there, so this module is intentionally self-contained and does NOT
    import anything from the fork.
    The duplication of set_seed / gc logic here is a deliberate trade-off.

Invoked from benchmarks/cuda.py via subprocess.
Saves results to a JSON file at the path given by --output.

NOTE: The upstream ChatterboxMultilingualTTS does not have generate_streaming.
TTFC is measured only when the method is available; otherwise ttfc_avg_ms and
ttfc_p95_ms are omitted from the output.
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--mtl-model-dir",
        default=None,
        help="Path to local MTL model weights directory (bypasses HuggingFace)",
    )
    parser.add_argument(
        "--turbo-model-dir",
        default=None,
        help="Path to local Turbo model weights directory (bypasses HuggingFace)",
    )
    ...
    has_streaming = hasattr(model, "generate_streaming")
    ...
    for _ in range(args.iterations):
        ...
        # TTFC — only when the upstream model supports generate_streaming
        if has_streaming:
            cuda_sync()
    ...
    if ttfc_times:
        result["ttfc_avg_ms"] = float(np.mean(ttfc_times))
        result["ttfc_p95_ms"] = float(np.percentile(ttfc_times, 95))
